...
class EnvSettings_ITP:
    def __init__(self, workspace_dir, data_dir, save_dir):
        ...
        self.dtb70_path = os.path.join(data_dir, 'dtb70_path')
        self.uavdt_path = os.path.join(data_dir, 'uavdt_path')
        self.visdrone2018_path = os.path.join(data_dir, 'visdrone2018_path')
        self.uav123_path = os.path.join(data_dir, 'uav123_path')
        self.uav123_10fps_path = os.path.join(data_dir, 'uav123_10fps_path')
        ...
        for attr in dir(settings):
            ...
            if not attr.startswith('__') and not callable(attr_val):
                if comment_str is None:
                    ...
                    f.write('    settings.{} = \'{}\'    # {}\n'.format(attr, attr_val, comment_str))
        f.write('\n    return settings\n\n')
        print(f"Proccess is completed {path}")
