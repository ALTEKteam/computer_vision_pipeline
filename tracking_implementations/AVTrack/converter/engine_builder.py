...
"""
TensorRT engine builder for AVTrack ONNX models.

Quick usage examples:
    python engine_builder.py
    python engine_builder.py --onnx output/onnx/avtrack.onnx --engine output/avtrack_fp16.engine --fp16
    python engine_builder.py --onnx output/onnx/avtrack.onnx --engine output/avtrack_fp32.engine --no-fp16 --workspace-gb 2.0

Notes:
- Run this script from the AVTrack root directory for default relative paths.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--onnx",
        type=Path,
        default=Path("output/onnx/avtrack.onnx"),
        help="Path to the ONNX model file.",
    )
    parser.add_argument(
        "--engine",
        type=Path,
        default=Path("output/avtrack_fp16.engine"),
