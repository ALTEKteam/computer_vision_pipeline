...
class Block(nn.Module):
    def __init__(
        ...
        self.active_score_module = nn.Linear(320, 1)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, z, x, is_distill):
        x = self.patch_embed(x)
        ...
        for i, blk in enumerate(self.blocks):
            if i > 1 and not is_distill:
                prob_active = (blk.active_score_module(x[:, :, 0].clone())).sigmoid_()
                idx, _ = torch.where(prob_active > 0.5)
                if len(idx) > 0:
                    if self.training:
                        x[idx] = blk(x[idx])
                    # block_list.append(x[idx])
                    else:
                        x = blk(x)
        ...
        aux_dict = {"attn": None, 'probs_active': probs_active}
